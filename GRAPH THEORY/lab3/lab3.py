...
class DirectedGraph:
    ...
    def dijkstra(self, start, end):
        ...
        if start not in self._dictOut or end not in self._dictOut:
            return float(
                'inf'), []  # if they don't exist, we can't calculate the path so we return infinite and an empty list
        ...
        prev = {v: None for v in
                vertices}  # dictionary with the precessors, initialize with None. Here we keep the previous node in the shortest path to the vertex
        ...
        while pq:  # while there are nodes in the heapq
            ...
            for v in self.parse_outbound_edges(u):
                ...
                if cost is not None and dist[u] + cost < dist[
                    v]:  # If there exists a valid edge and a new path to v through u is shorter than the previous one
        ...
        while v is not None:
            path.append(
                v)  # Each node is appended while tracing backwards from the end to the start using the prev dictionary, and then the list is reversed to obtain the correct order from start to end.
            v = prev[v]
        path.reverse()

        return dist[
            end], path  # finally it returns the total minimal distance and the list of nodes in the shortest path
...
def menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            n1 = 7
            m1 = 20
            rgraph1 = DirectedGraph.random_graph(n1,
                                                 m1)  # the statement says that is has to be a graph with 7 vertices and 20 edges
            if m1 > n1 * n1:
